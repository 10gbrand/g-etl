...
class PipelineScreen(Screen):
    ...
    def get_selected_datasets(self) -> list[Dataset]:
        ...
        return [
            row.dataset for row in self.query(DatasetRow) if row.selected and row.dataset.enabled
        ]
    ...
    async def run_datasets(self, datasets: list[Dataset]) -> None:
        ...
        if run_extract:
            ...
            if datasets_to_extract:
                self.log_message(
                    f"Startar parallell extraktion av {len(datasets_to_extract)} dataset(s)..."
                )
            else:
                self.log_message(
                    "Alla valda datasets har befintliga parquet-filer, hoppar extraktion"
                )
