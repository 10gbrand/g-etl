...
class MssqlPlugin(SourcePlugin):
    ...
    def extract(
        ...
        self._log("Ansluter till MSSQL...", on_log)
        ...
        try:
            ...
            if not rows:
                self._log("Inga rader hittades", on_log)
                return ExtractResult(
                    success=True,
                    rows_count=0,
            ...
            if processed_rows:
                ...
                for i in range(0, len(processed_rows), batch_size):
                    ...
                    insert_sql = (
                        f"INSERT INTO raw.{table_name} ({columns_str}) VALUES ({placeholders})"
                    )
                    conn.executemany(insert_sql, batch)

                    progress = 0.8 + (
                        0.15 * min(i + batch_size, len(processed_rows)) / len(processed_rows)
                    )
                    self._progress(
                        progress,
                        f"Infogar rader ({i + len(batch)}/{len(processed_rows)})...",
                        on_progress,
                    )
