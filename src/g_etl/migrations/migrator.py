...
class Migrator:
    ...
    def _get_applied_versions(self) -> set[str]:
        ...
        try:
            result = self.conn.execute(f"SELECT version FROM {self.MIGRATIONS_TABLE}")
    ...
    def run_init_migrations(
        ...
        """Kör infrastruktur-migrationer (001-003).

        Dessa skapar extensions, scheman och makron som krävs
        innan pipelinen kan köras. Används av både CLI och TUI.
        ...
        for migration in self.discover_migrations():
            ...
            if self.is_template_migration(migration):
                continue
            ...
            if migration.status == MigrationStatus.APPLIED:
                if on_log:
                    on_log(f"  ✓ {migration.name} (redan körd)")
                continue

            if on_log:
                on_log(f"  Kör {migration.name}...")

            # Kör migrationen statement för statement
            statements_run = 0
            try:
                for statement in migration.up_sql.split(";"):
                    # Ta bort kommentarlinjer
                ...
                applied.append(migration.version)
                if on_log:
                    on_log(f"  ✓ {migration.name}: {statements_run} statements")

            except Exception as e:
                # Redan registrerad, men kördes ändå
                if "duplicate" in str(e).lower() or "unique" in str(e).lower():
                    ...
                    if on_log:
                        on_log(f"  ✗ {migration.name}: {e}")

        success = len(failed) == 0
        message = f"Körde {len(applied)} init-migrering(ar)"
        if failed:
            message += f", {len(failed)} misslyckades"
