-- Staging: Skyddsvarda_statliga_skogar
-- Skyddsvärda statliga skogar från Naturvårdsverket
...
CREATE OR REPLACE TABLE staging.skyddsvarda_statliga_skogar AS
WITH source_data AS (
    SELECT * FROM raw.skyddsvarda_statliga_skogar
...
SELECT
    ...
    -- MD5-hash av käll-ID (Naturvårdsverket använder NVRID)
    MD5(CAST(s.ID AS VARCHAR)) AS _source_id_md5,
