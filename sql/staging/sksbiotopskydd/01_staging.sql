...
--   _h3_index        - H3-cell för centroid (res 13, ~43m²)
--   _h3_cells        - Alla H3-celler inom ytan (res 11, ~2149m²)
--   _a5_index        - A5-cell (reserverad för framtida användning)
--
-- OBS: Geometrikolumnen normaliseras till 'geom' av pipeline_runner
--      innan denna SQL körs. Alternativa namn hanteras automatiskt.

CREATE OR REPLACE TABLE staging.sksbiotopskydd AS
WITH source_data AS (
    SELECT * FROM raw.sksbiotopskydd
...
SELECT
    ...
    MD5(CAST(s.objectid AS VARCHAR)) AS _source_id_md5,
